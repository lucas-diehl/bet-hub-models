...
- **Idea #1 (scenario-conditioned correlation) — evidence gathered 2026-09, real and dramatic; simulator NOT yet built.** A full synthetic scenario-injection sim needs play-by-play data we don't ingest. Instead, tested the tractable question directly on data we have: **does realized correlation depend on ACTUAL game script?** (`tests/validate_game_script_correlation.R`, WNBA box scores, high-minute players, split by final margin.) Result — **the correlation SIGN FLIPS by script**:
  | | close game (≤8) | blowout (≥18) |
  |---|---|---|
  | opponent pairs | +0.041 | **−0.068** |
  | same-team pairs | −0.001 | **+0.076** |

  A single flat factor (what `correlate.R` uses, before *and* after the calibration in item 13) **cannot represent a sign flip** — this is concrete, measured proof the "hidden, script-dependent correlation" Blickle describes is real for us, not speculative. **This makes idea #1 a high-priority, evidenced next project**, not a maybe. What's still needed to actually build it: (a) a pre-game script-probability estimate (e.g., Vegas spread → P(blowout), WNBA already ingests Vegas lines via `team_env.R`), (b) a `slate_sim.R` architecture change to draw a discrete script regime **per simulation** and switch loadings conditionally — currently `load`/`team_load` are fixed scalars computed once per player, not something that varies draw-to-draw. That architecture change is the properly-scoped follow-up; not attempted here.
