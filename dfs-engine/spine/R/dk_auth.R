...
sync_ownership <- function(extra_ids = NULL) {
  ...
  # A contest counts as logged once it has any actual_pct row. Do NOT additionally require
  # player_id > 0: unmatched players get a deterministic NEGATIVE surrogate id (see
  # surrogate_player_id in db.R), which the slate pool assigns identically, so negative
  # ids are normal and joinable — treating them as unresolved re-downloads ~110 settled
  # contests from DK on every run for no benefit.
